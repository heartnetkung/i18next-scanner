    ...
    value: function set(key) {
      ...
      lngs.forEach(function (lng) {
        ...
        if (!_lodash["default"].isPlainObject(resLoad)) {
          ...
          resLoad = _this5.resStore[lng][ns] = {};
          resScan = _this5.resScan[lng][ns] = {}; //this.error(`${chalk.yellow(JSON.stringify(ns))} does not exist in the namespaces (${chalk.yellow(JSON.stringify(this.options.ns))}): key=${chalk.yellow(JSON.stringify(key))}, options=${chalk.yellow(JSON.stringify(options))}`);
          //return;
        }
